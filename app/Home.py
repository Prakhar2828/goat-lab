...
The score difference is only **{margin:.3f} points on a 100-point scale**, so the
default result is extremely close.
"""
)
...
always remained 50% / 40% / 10%.

The published run used:

- 250,000 samples
- Random seed 23
- Within-section concentration 100

**My result** is the single published default setup.

**The stress-test result** asks how often each player leads when those internal
priorities are changed in many reasonable ways.
"""
    )
...
with st.expander("Why another reasonable scoring method can change the winner"):
    ...
    if scale_frame.empty:
        st.warning("Scaling-sensitivity results are unavailable.")
    else:
...
st.divider()
st.header("Explore the full analysis")

st.write(
    "GOAT Lab contains nine interactive pages covering different parts of the "
    "Jordan–LeBron comparison."
)

st.info(
    "📱 **On a phone:** tap the » icon in the top-left to open the full page menu.  \n"
...
st.page_link(
    "pages/4_Team_Context.py",
    label="🤝 Explore team and winning context",
    width="stretch",
)

st.page_link(
    "pages/5_Skill_Profile.py",
    label="🏀 Compare their skill profiles",
    width="stretch",
)

st.page_link(
    "pages/6_Cultural_Impact.py",
    label="🌍 Explore cultural impact",
    width="stretch",
)
